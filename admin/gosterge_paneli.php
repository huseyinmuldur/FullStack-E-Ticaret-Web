<?php

include '../temelbolumler/connect.php';

session_start();

$admin_id = $_SESSION['admin_id'];

if(!isset($admin_id)){
   header('location:admin_girisi.php');
}

?>

<!DOCTYPE html>
<html lang="en">
<head>
   <meta charset="UTF-8">
   <meta http-equiv="X-UA-Compatible" content="IE=edge">
   <meta name="viewport" content="width=device-width, initial-scale=1.0">
   <title>Gösterge Paneli</title>

   <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.1.1/css/all.min.css">

   <link rel="stylesheet" href="../css/admin_style.css">

</head>
<body>

<?php include '../temelbolumler/admin_header.php'; ?>

<section class="dashboard">

   <h1 class="heading">Gösterge PANELİ</h1>

   <div class="box-container">

      <div class="box">
         <h3>HOŞGELDİNİZ</h3>
         <p><?= $fetch_profile['isim']; ?></p>
         <a href="profil_guncelleme.php" class="btn">Profili Güncelle</a>
      </div>

      <div class="box">
         <?php
            $total_pendings = 0;
            $select_pendings = $conn->prepare("SELECT * FROM `siparisler` WHERE odeme_durumu = ?");
            $select_pendings->execute(['pending']);
            if($select_pendings->rowCount() > 0){
               while($fetch_pendings = $select_pendings->fetch(PDO::FETCH_ASSOC)){
                  $total_pendings += $fetch_pendings['toplam_ucret'];
               }
            }
         ?>
         <h3><?= $total_pendings; ?><span></span><span>TL</span></h3>
         <p>Toplam Bekleyen Siparişler</p>
         <a href="siparisler.php" class="btn">Siparişleri Gör</a>
      </div>

      <div class="box">
         <?php
            $total_completes = 0;
            $select_completes = $conn->prepare("SELECT * FROM `siparisler` WHERE odeme_durumu = ?");
            $select_completes->execute(['completed']);
            if($select_completes->rowCount() > 0){
               while($fetch_completes = $select_completes->fetch(PDO::FETCH_ASSOC)){
                  $total_completes += $fetch_completes['toplam_ucret'];
               }
            }
         ?>
         <h3><?= $total_completes; ?><span></span><span>TL</span></h3>
         <p>Tamamlanan Siparişler</p>
         <a href="siparisler.php" class="btn">Siparişleri Gör</a>
      </div>

      <div class="box">
         <?php
            $select_orders = $conn->prepare("SELECT * FROM `siparisler`");
            $select_orders->execute();
            $number_of_orders = $select_orders->rowCount()
         ?>
         <h3><?= $number_of_orders; ?></h3>
         <p>Verilen Siparişler</p>
         <a href="siparisler.php" class="btn">Siparişleri Gör</a>
      </div>

      <div class="box">
         <?php
            $select_products = $conn->prepare("SELECT * FROM `urunler`");
            $select_products->execute();
            $number_of_products = $select_products->rowCount()
         ?>
         <h3><?= $number_of_products; ?></h3>
         <p>Eklenen Ürünler</p>
         <a href="urunler.php" class="btn">Ürünlere Bak</a>
      </div>

      <div class="box">
         <?php
            $select_users = $conn->prepare("SELECT * FROM `kullanicilar`");
            $select_users->execute();
            $number_of_users = $select_users->rowCount()
         ?>
         <h3><?= $number_of_users; ?></h3>
         <p>Normal Kullanıcılar</p>
         <a href="kullanicilar.php" class="btn">Kullanıcılara Bak</a>
      </div>

      <div class="box">
         <?php
            $select_admins = $conn->prepare("SELECT * FROM `adminler`");
            $select_admins->execute();
            $number_of_admins = $select_admins->rowCount()
         ?>
         <h3><?= $number_of_admins; ?></h3>
         <p>Admin Kullanıcılar</p>
         <a href="admin_hesabi.php" class="btn">Adminlere Bak</a>
      </div>

      <div class="box">
         <?php
            $select_messages = $conn->prepare("SELECT * FROM `mesajlar`");
            $select_messages->execute();
            $number_of_messages = $select_messages->rowCount()
         ?>
         <h3><?= $number_of_messages; ?></h3>
         <p>Yeni Mesajlar</p>
         <a href="mesajlar.php" class="btn">Mesajlara Bak</a>
      </div>

   </div>

</section>

<script src="../js/admin_script.js"></script>
   
</body>
</html>